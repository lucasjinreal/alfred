...
def draw_box_without_score(img, boxes, classes=None, is_show=False):
    ...
    """
    warnings.warn(
        "this method is deprecated, using visiualize_det_cv2 instead",
        DeprecationWarning,
    )
    assert isinstance(
        boxes, np.ndarray
...
def visualize_det_cv2_style0(
    ...
    if cls_colors and classes:
        assert len(cls_colors) == len(
            classes
        ), "cls_colors must be same with classes length if you specific cls_colors."
    ...
    for i in range(detections.shape[0]):
        ...
        if cls_id != background_id:
            ...
            if score > thresh:
                ...
                ((txt_w, txt_h), _) = cv2.getTextSize(text_label, font, font_scale, 1)
    ...
    if counter_on:
        ...
        for k, v in cc.items():
            txt += "{}: {}\n".format(k, v)
        put_txt_with_newline(
            img, txt, counter_pos, font, 1.9, (0, 255, 0), 2, cv2.LINE_AA
        )
...
def visualize_det_cv2_part(
        ...
        if cls_id != background_id:
            if scores is not None:
                    ...
                    if force_color:
                        if random:
                            ...
                            if cls_id > len(force_color) - 1:
                                unique_color = force_color[
                                    min(cls_id, len(force_color) - 1)
                                ]
                            else:
                ...
                if force_color:
                    ...
                    if track_ids:
                        # if track, identify color by track_id
                        unique_color = colors(track_ids[i], True)
                    else:
                        unique_color = colors(cls_id, True)
                x1, y1, x2, y2 = 0, 0, 0, 0
                if mode == "xyxy":
    ...
    if show_time > 0:
        cv2.putText(
            img,
            f"{round(1/show_time, 1)}FPS {show_time*1000}ms",
            (46, 60),
            font,
            font_scale + 0.6,
            (125, 209, 46),
...
def draw_one_3d_box_cv2(
    ...
    if tlwhy_format:
        ...
        assert (
            calib_cam_to_img_p2
        ), "You should provide calibration matrix, convert camera to image coordinate."
        center = box_3d[0:3]
        dims = box_3d[3:6]
        ...
        for i in [1, -1]:
            for j in [1, -1]:
                for k in [0, 1]:
                    ...
                    point[0] = (
                        center[0]
                        + i * dims[1] / 2 * np.cos(-rot_y + np.pi / 2)
                        + (j * i) * dims[2] / 2 * np.cos(-rot_y)
                    )
                    point[2] = (
                        center[2]
                        + i * dims[1] / 2 * np.sin(-rot_y + np.pi / 2)
        ...
        for i in range(4):
            ...
            cv2.line(
                img, (point_1_[0], point_1_[1]), (point_2_[0], point_2_[1]), color, 1
            )
